...
- **Dispatch-Source-Keep**: commits with `Dispatch-Source-Keep: true` trailer auto-resolve conflicts with `--strategy-option theirs`. Use for generated files where source version should always win.
...
| Generated file conflict on cherry-pick | Add `--trailer "Dispatch-Source-Keep=true"` to the commit; source version auto-accepted on conflict |
