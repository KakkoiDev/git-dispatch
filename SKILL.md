...
- **Auto-resolve**: when `dispatch.postApply` is configured, all cherry-pick operations (`apply`, `cherry-pick`, `_cherry_pick_with_trailers`) retry with `--strategy-option theirs` before showing conflict. This handles generated file conflicts that will be regenerated by postApply.
...
| Generated file conflict on any cherry-pick | Auto-resolved with `--theirs` when `dispatch.postApply` is configured (applies to `apply`, `cherry-pick`, and trailer rewrites) |
